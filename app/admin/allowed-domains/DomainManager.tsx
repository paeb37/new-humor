...
export default function DomainManager({ domains }: { domains: Domain[] }) {
  const [showCreateModal, setShowCreateModal] = useState(false)
  const [editingDomain, setEditingDomain] = useState<Domain | null>(null)
  const [searchTerm, setSearchTerm] = useState('')

  const filteredDomains = domains.filter((d) =>
    d.apex_domain.toLowerCase().includes(searchTerm.toLowerCase())
  )
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {filteredDomains.map((domain) => (
              ...
              >
                <td style={{ color: '#f4f4f5', padding: '1rem' }}>{domain.apex_domain}</td>
...
function DomainModal({
  ...
  const [apex_domain, setApexDomain] = useState(domain?.apex_domain || '')

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const formData = new FormData()
    formData.append('apex_domain', apex_domain)
